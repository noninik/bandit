...
AGENTS = {
    "router": {
        ...
        "icon": "🧠",
        "color": "#58a6ff",
        "prompt": """Ты — диспетчер AI-команды. Определи лучшего агента.

Агенты:
- scanner: сканер соцсетей, поиск болей в Reddit/YouTube/Twitter
- researcher: глубокий анализ рынка и ниши
- idea_generator: генерация бизнес-идей из болей
- business_plan: детальный бизнес-план
- strategist: стратегия роста и масштабирования
- marketer: маркетинг, контент, воронки
- developer: техническая архитектура, MVP
- sales: скрипты продаж, переговоры

Правила:
- "ниша", "рынок", "тренды" → researcher
- "боли", "проблемы", "соцсети", "Reddit" → scanner
- "идеи", "что создать" → idea_generator
- "план", "финансы", "запуск" → business_plan
- "стратегия", "рост" → strategist
- "реклама", "контент", "лендинг" → marketer
- "код", "приложение", "MVP" → developer
- "продажи", "скрипт", "клиенты" → sales
...
{"agent": "id", "reason": "почему"}"""
    ...
    "scanner": {
        ...
        "icon": "📡",
        "color": "#39d2c0",
        "prompt": """РОЛЬ: Лучший аналитик социальных медиа, 10 лет опыта.

ЗАДАЧА: Сканируй соцсети и найди РЕАЛЬНЫЕ боли людей.
...
Сортировка: Частота x Готовность платить

[ЗОЛОТЫЕ ВОЗМОЖНОСТИ]
3 ниши с болью 8+/10 и готовностью платить 7+/10.
Для каждой: что создать и цена.

Будь конкретным. Реальные названия и цифры. Отвечай на языке пользователя."""
    },
    "idea_generator": {
        "name": "Генератор идей",
        "icon": "💡",
...
ЗАДАЧА: 5 бизнес-идей от лучшей к худшей.
...
Реализуемо одним человеком, бюджет до $500. Отвечай на языке пользователя."""
    },
    "business_plan": {
        "name": "Бизнес-планировщик",
        "icon": "📋",
        "color": "#8b5cf6",
        "prompt": """РОЛЬ: Бизнес-консультант уровня McKinsey, 20 лет опыта.

СТРУКТУРА:

[РЕЗЮМЕ] 3 предложения.
[ПРОБЛЕМА] Боль + масштаб + почему текущие решения плохие.
[РЕШЕНИЕ] Продукт + 5 функций + отличия.
[РЫНОК] TAM/SAM/SOM + аватар клиента.
[БИЗНЕС-МОДЕЛЬ] Монетизация + 3 тарифа + unit-экономика (CAC, LTV, маржа).
[MVP 14 ДНЕЙ] По дням.
[МАРКЕТИНГ] 4 недели: канал, действие, бюджет, результат.
[ФИНАНСЫ] Месяц 1-3, 4-6, 7-12: выручка, расходы, прибыль. Точка безубыточности.
[РИСКИ] 5 рисков: вероятность, влияние, митигация.
[ДОРОЖНАЯ КАРТА] Месяц 1, 3, 6, 12.
[СЛЕДУЮЩИЙ ШАГ] Что сделать СЕГОДНЯ за 1 час.

Реалистично для 1 человека с $500. Конкретные цифры. Отвечай на языке пользователя."""
    },
    "researcher": {
        "name": "Исследователь",
        "icon": "🔍",
        "color": "#3fb950",
        "prompt": """РОЛЬ: Аналитик рынка, 15 лет опыта.

[СКАНИРОВАНИЕ] Размер ниши в $, стадия, рост %.
[АУДИТОРИЯ] 3 сегмента: демография, боли, бюджет, площадки.
[КОНКУРЕНТЫ] 5 штук: название, выручка, сильные/слабые стороны, цены.
[ТРЕНДЫ] 5 трендов с цифрами.
[БОЛИ] 5 нерешённых проблем с оценкой готовности платить.
[ВЫВОД] Входить или нет + конкретный план.
...
Реальные компании и цифры. Отвечай на языке пользователя."""
    ...
    "strategist": {
        ...
        "icon": "🎯",
        "color": "#f59e0b",
        "prompt": """РОЛЬ: Бизнес-стратег, 50+ стартапов от идеи до $10M ARR.

[ПРОБЛЕМА] 1 предложение + масштаб.
[РЕШЕНИЕ] Продукт + 3 отличия + почему сейчас.
[МОДЕЛЬ] Монетизация + 3 тарифа + upsell.
[UNIT-ЭКОНОМИКА] CAC, LTV, маржа, payback.
[MVP] 2 недели по дням → первый платящий клиент.
[GROWTH] 0→100, 100→1000, 1000→10000.
[РИСКИ] 3 риска + план B.

Конкретные цифры и примеры. Отвечай на языке пользователя."""
    },
    "marketer": {
        "name": "Маркетолог",
        "icon": "📢",
...
[КОМПОНЕНТЫ] Модули и связи.
[КОД] Рабочий код с комментариями.
[API] Бесплатные API.
[ДЕПЛОЙ] Пошагово до URL.
[СРОКИ] Задача → часы.

Python + Flask, бесплатный хостинг. Отвечай на языке пользователя."""
    },
    "sales": {
        "name": "Продажник",
        "icon": "🤝",
        "color": "#ef4444",
        "prompt": """РОЛЬ: Директор по продажам, 15 лет, 1000+ сделок.

[ПРОДУКТ] Ценность в 1 предложении.
[АВАТАР] Имя, должность, мотивация, страхи.
[ПЕРВЫЙ КОНТАКТ] 3 скрипта: email, LinkedIn, DM.
[ПРЕЗЕНТАЦИЯ] 10 слайдов.
[ВОЗРАЖЕНИЯ] 10 штук + ответы.
[ЗАКРЫТИЕ] 5 техник с фразами.
[FOLLOW-UP] 5 писем: день 1, 3, 7, 14, 30.
...
def fullcycle():
    ...
    steps = [
        ("scanner", "Просканируй все соцсети и найди боли аудитории в нише: " + niche + ". Реальные subreddit-ы, каналы, цитаты."),
        ("idea_generator", None),
        ("business_plan", None)
    ]
    ...
    for i, (agent_id, custom_msg) in enumerate(steps):
        ...
        try:
            ...
            if custom_msg:
                ...
                msg = "На основе данных выше выполни задачу для ниши: " + niche + ". Выбери лучшую идею и сделай детальный результат."
...
def get_templates():
    templates = [
        ...
        {"title": "/plan", "prompt": "Бизнес-план для: ", "desc": "📋 Детальный план"},
        {"title": "/fullcycle", "prompt": "", "desc": "🚀 Скан + Идеи + План"},
        {"title": "/research", "prompt": "Исследуй нишу: ", "desc": "🔍 Анализ рынка"},
